// yanked from https://github.com/Nushaine/calculateLinearGradient-FigmaAPI/blob/main/linearGradient.ts
...
export function RGBAObject2String(
...
export function calculateIntersection(
  ...
  const p = { x: round(px, 2), y: round(py, 2) };
...
export function getCorners(component: GradientComponent, elemRotation: number) {
...
export function createLinearGradient(
  ...
  fill.gradientStops.map(
    (color: {
      ...
      const colorObj = { r: color.color.r, g: color.color.g, b: color.color.b };
      const fillOpacity = fill.opacity === undefined ? 1 : fill.opacity;
      const formattedColor = RGBAObject2String(
        colorObj,
